...
def _s9(p,d):
    ...
    _tx(s,"Confiam em nós",0.3,0.2,5,0.35,14,True,it=True,c=C.white)
    a=_assets()
    logos_path = os.path.join(a,"logos_clientes.png") if a else None
    if logos_path and os.path.exists(logos_path):
        _img(s,logos_path,0.3,0.6,5.0,4.6)
    else:
        _rt(s,0.3,0.6,5.0,4.6,C.white)
        ...
        cols=4;cw=1.15;ch=0.5;gx=0.05;gy=0.05
        for i,nm in enumerate(clientes_ref):
            col=i%cols;row=i//cols
            cx=0.5+col*(cw+gx);cy=0.8+row*(ch+gy)
    ...
    # Parceiros
    _tx(s,"Parceiros",0.3,5.35,5,0.3,12,True,c=C.white)
    _rt(s,0.3,5.7,5.0,0.7,RGBColor(0x20,0x30,0x50))
    _tx(s,"Moura  ·  Ambar Energia  ·  Itaipu",0.5,5.82,4.6,0.4,13,True,c=C.white,al=PP_ALIGN.CENTER)
    # Logo grande
